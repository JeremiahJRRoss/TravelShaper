...
- `otel_routing.build_tracer_provider()` configures a `TracerProvider`. For Phoenix, it uses `BatchSpanProcessor` with `OTLPSpanExporter`. For Arize, it uses `arize.otel.register()` which returns a fully configured provider
